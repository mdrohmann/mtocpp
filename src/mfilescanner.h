...
struct RunMode {
	RunMode() :
			...
			remove_first_arg_in_abstract_methods(
					REMOVE_FIRST_ARG_IN_ABSTRACT_METHODS),
			...
			generate_subfunction_documentation(
					GENERATE_SUBFUNTION_DOCUMENTATION) {
	}
...
private:
	...
	void print_access_specifier(AccessEnum & access, MethodParams & mp,
			PropParams & pp);
	...
	void extract_typen(DocuBlock & db, std::string & typen,
			bool remove = false);
	...
	void write_docu_list(const DocuList & list, const std::string & item_text,
			const AltDocuList & alternative, bool, const std::string separator,
			const std::string docu_list_name);

	void write_docu_listmap(const DocuListMap & listmap,
			const std::string & text, const AltDocuListMap & altlistmap);
