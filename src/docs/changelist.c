 ...
 * - Changed the \c SHOW_FILES default value in the doxygen configuration file from "NO" to "YES".
 * - Bugfix: The setting \c EXTRA_PACKAGES in the doxygen configuration file was given the wrong path format,
 * as latex follows the unix file separation using only forwardslashes "/", so the inclusion failed on Windows platforms.
 * We fixed this by passing the correctly transformed path. Also a new placeholder "_FileSep_" which is being processed by MatlabDocMaker
 * (any any tools to come) and set to the correct file separator character for your platform.
 *
