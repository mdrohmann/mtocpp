...
const char * AccessEnumNames[] = { stringify( Public ), stringify( Protected ),
		stringify( Private ), "Specific classes" };

const char * ClassPartNames[] = { stringify( InClassComment ),
		stringify( Header ), stringify( Method ), stringify( AtMethod ),
		stringify( MethodDeclaration ), stringify( Property ),
		stringify( Event ) };
...
void MFileScanner::update_method_params(const std::string & methodname) {
	...
	try {
		...
		MFileScanner scanner(*fcin, oss, filename, cscan_.get_conffile(),
				methodParamsMode);
		scanner.execute();
		methodparams_ = scanner.getMethodParams();
	} catch (const ifstream::failure & e) {
		std::cerr << "MTOCPP Warning: No method params for @-function "
				<< methodname << " found!\n";
	}
}
	...
	if (is_class_ && (cfuncname_ == classname_))
		...
		if (returnlist_.size() == 0)
			...
			for (unsigned int i = 0; i < returnlist_.size(); ++i) {
				...
				if (runMode_.print_return_value_name - (returnlist_.size() == 1)
						> 0)
					fout_ << "," << returnlist_[i];
				fout_ << "> ";
		...
		for (unsigned int i = 0; i < paramlist_.size(); ++i) {
			...
			if (defvalue.empty()) {
				if (is_default_necessary && paramlist_[i] != "varargin") {
					...
					oss << "Optional parameter '" << paramlist_[i]
							<< "' of method '" << cfuncname_
...
void MFileScanner::print_function_synopsis() {
	...
	if (is_class_
			&& (class_part_ == Method || class_part_ == AtMethod
 ...
 *
 * @change{1,5,dw,2013-03-27} Also accepting quoted access type specifications, e.g. Access='private'. Thanks to Pete for pointing out!
 *
 */
MFileScanner::MFileScanner(istream & fin, ostream & fout,
		const std::string & filename, const std::string & conffilename,
	...
	while (ppos != string::npos) {
		...
		if (directory[0] == '+') {
			...
			is_class_ = true;
			if (classname_
					!= fnname_.substr(fnname_.find_last_of('/') + 1,
							classname_.size())) {
	...
	if (cscan_.vars_.find(string("COPY_TYPIFIED_FIELD_DOCU"))
			!= cscan_.vars_.end()) {
		if (cscan_.vars_[string("COPY_TYPIFIED_FIELD_DOCU")][0]
				== string("true")) {
	...
	if (cscan_.vars_.find(string("AUTO_ADD_PARAMETERS"))
			!= cscan_.vars_.end()) {
	...
	if (cscan_.vars_.find(string("AUTO_ADD_CLASS_PROPERTIES"))
			!= cscan_.vars_.end()) {
		if (cscan_.vars_[string("AUTO_ADD_CLASS_PROPERTIES")][0]
				== string("true")) {
	...
	if (cscan_.vars_.find(string("REMOVE_FIRST_ARG_IN_ABSTRACT_METHODS"))
			!= cscan_.vars_.end()) {
		if (cscan_.vars_[string("REMOVE_FIRST_ARG_IN_ABSTRACT_METHODS")][0]
				== string("true")) {
	...
	if (cscan_.vars_.find(string("ENABLE_OF_TYPE_PARSING"))
			!= cscan_.vars_.end()) {
		if (cscan_.vars_[string("ENABLE_OF_TYPE_PARSING")][0]
				== string("true")) {
	...
	if (cscan_.vars_.find(string("VOID_TYPE_IN_RETURN_VALUES"))
			!= cscan_.vars_.end()) {
		if (cscan_.vars_[string("VOID_TYPE_IN_RETURN_VALUES")][0]
				== string("true")) {
	...
	if (cscan_.vars_.find(string("PRINT_RETURN_VALUE_NAME"))
			!= cscan_.vars_.end()) {
	...
	if (cscan_.vars_.find(string("GENERATE_SUBFUNTION_DOCUMENTATION"))
			!= cscan_.vars_.end()) {
		if (cscan_.vars_[string("GENERATE_SUBFUNTION_DOCUMENTATION")][0]
				== string("true")) {
...
void MFileScanner::write_docu_block(const DocuBlock & block_orig) {
	...
	for (unsigned int i = 0; i < block.size(); i += 1) {
		...
		for (string::size_type i = 0; j < s.size(); i = j) {
			...
			if (s[i] == '@') {
				if (s.substr(i + 1, 4) == "code"
						|| s.substr(i + 1, 8) == "verbatim")
					not_verbatim = false;
				else if (s.substr(i + 1, 7) == "endcode"
						|| s.substr(i + 1, 11) == "endverbatim")
					not_verbatim = true;
...
void MFileScanner::write_docu_list(const DocuList & list,
		const string & item_text, const AltDocuList & alternative,
		bool add_undocumented = false, const string separator = string(),
	...
	for (; lit != list.end(); ++lit) {
		...
		if (name.substr(0, 6) == std::string("unused")
				&& name.find_first_not_of("0123456789", 7)
						== std::string::npos) {
			ostringstream oss;
			oss << "* " << item_text << " " << name << separator
					<< " Marked as \"~\" in original m-file.\n  ";
			fout_ << oss.str();
			continue;
		}
		ostringstream oss;
		...
		if (block.size() == 1) {
			...
			if (typeof_length > 0
					&& block[0].find_first_of(" ", typeof_length)
							== std::string::npos)
		...
		if (block.empty() || use_alternative) {
			...
			if (alit == alternative.end() || (*alit).second.empty()) {
				...
				if (!docu_list_name.empty()
						&& runMode_.copy_typified_field_docu) {
				...
				if (param_type_map_entry != param_type_map_.end()) {
					...
					fout_ << oss.str() << "@copybrief "
							<< (*param_type_map_entry).second << "::" << temp
							<< "\n  ";
				} else {
					if (add_undocumented) {
...
void MFileScanner::write_docu_listmap(const DocuListMap & listmap,
		const string & text, const AltDocuListMap & altlistmap) {
	...
	if (!listmap.empty()) {
		...
		for (; mit != listmap.end(); ++mit) {
			...
			write_docu_list((*mit).second, "@arg \\c",
					...
					runMode_.auto_add_fields, "&nbsp;&mdash;&nbsp;",
					(*mit).first);
		}
//    fout_ << "* </TABLE>\n  ";
...
void MFileScanner::end_of_property_doc() {
	if (undoced_prop_) {
		...
		fout_ << propertyparams_.ccprefix() << typen << " "
				<< property_list_.back();
	...
	if (!cfuncname_.empty()) {
		...
		if (runMode_.mode != RunMode::ParseMethodParams && docuheader_.empty()
			...
			try {
				...
				MFileScanner scanner(*fcin, oss, filename,
						cscan_.get_conffile(), paramsMode);
				scanner.execute();
				param_list_ = scanner.getParamList();
			} catch (const ifstream::failure & e) {
				std::ostringstream oss;
				oss << "Class " << classname_
						<< " misses definition of function " << cfuncname_
						<< "!";
				print_warning(oss.str());
			}
		}
...
const std::string & MFileScanner::extract_default(DocuBlock & db,
		std::string & defvalue) {
	typedef DocuBlock::iterator DBIt;
		...
		if (found == std::string::npos) {
			...
			if (found != std::string::npos) {
				...
				line = line.substr(0, found) + "@b Default: "
						+ line.substr(found + 9, end - found - 9) + " "
						+ line.substr(end);
			}
		}
	}
...
void MFileScanner::get_default(const std::string & paramname,
		std::string & defvalue) {
...
void MFileScanner::get_typename(const std::string & paramname,
		std::string & typen, std::string voidtype) {
	typedef DocuList::iterator DLIt;
...
void MFileScanner::extract_typen(DocuBlock & db, std::string & typen,
		bool remove) {
	int linenr = 1;
	typedef DocuBlock::iterator DBIt;
	for (DBIt dit = db.begin(); dit != db.end(); ++dit, ++linenr) {
		...
		if (found != std::string::npos) {
			...
			size_t typenend = line.find_first_of(" \n\0", typenstart);
 ...
 *
 * @change{1,5,dw,2013-03-27} Fixed URL for matlab properties description
 *
 */
void MFileScanner::add_property_params_info() {
	...
	if (propertyparams_.dependent) {
		...
		docuextra_.push_back(
				std::string(
						"@note This property has the MATLAB attribute @c Dependent set to true.\n"));
	}
	if (propertyparams_.setObservable) {
		any_property_set = true;
		docuextra_.push_back(
				std::string(
						"@note This property has the MATLAB attribute @c SetObservable set to true.\n"));
	}
	if (propertyparams_.abstr) {
		any_property_set = true;
		docuextra_.push_back(
				std::string(
						"@note This property is an @em abstract property without implementation.\n"));
	}
		...
		for (unsigned int i = 0; i < docubody_.size(); ++i) {
			...
			if (pos_begin_copy != std::string::npos) {
				...
				if (pos_word_begin != std::string::npos) {
					size_t pos_word_end = docubody_[i].find_first_of("(",
							pos_word_begin);
					if (pos_word_end != std::string::npos) {
						std::string func = docubody_[i].substr(pos_word_begin,
								pos_word_end - pos_word_begin);
...
// end function
	...
	if ((!docuheader_.empty() || runMode_.auto_add_class_properties)
			&& (is_first_function_
					|| runMode_.generate_subfunction_documentation)) {
		...
		if (!skip_parameters) {
			...
			// return values
			if (!return_list_.empty() && !is_constructor && !is_getter_
					&& !is_setter_) {
				fout_ << "*\n  ";
			...
			if (runMode_.print_fields) {
				// required fields
				write_docu_listmap(required_list_, "@par Required fields of ",
						cscan_.field_docu_);

				// optional fields
				write_docu_listmap(optional_list_, "@par Optional fields of ",
						cscan_.field_docu_);

				// return fields
				write_docu_listmap(retval_list_, "@par Generated fields of ",
						cscan_.field_docu_);
			}
		}
#ifdef DEBUG
...
void MFileScanner::extract_default_argument_of_inputparser(
...
void MFileScanner::postprocess_unused_params(std::string & param,
		DocuList & doculist) {
	if (param == std::string("~")) {
		int counter = 1;
		bool found = true;
...
void MFileScanner::handle_param_list_for_varargin() {
	...
	if (has_varargin) {
		...
		for (; last_param_tmp_it != param_list_.end(); last_param_tmp_it++) {
			...
			bool hasUserDefault = (extract_default(last_param_item_tmp.second,
					defval_temp) != "");
			...
			if (pvit == varargin_parser_values_.end()
				...
				if (!defaultval.empty() && !hasUserDefault)
					optionalParams[last_paramname_tmp].push_back(
							string("     ( @b Default: <tt>") + defaultval
									+ "</tt> )\n");
				...
				if (!defaultval.empty() && !hasUserDefault)
					mappedParams[last_paramname_tmp].push_back(
							string("     ( @b Default: <tt>") + defaultval
									+ "</tt> )\n");
		...
		for (ItType pvit = varargin_parser_values_.begin();
				pvit != varargin_parser_values_.end(); ++pvit) {
		...
		if (req_param_size + opt_param_size + mapped_param_size > 0) {
			...
			*oss << "@code " << cfuncname_ << " ( ";
			...
			for (unsigned int i = 0; i < mapped_param_size; ++i) {
				...
				first = 0;
				*oss << '"' << mappedParams.at(i).first << "\", "
						<< mappedParams.at(i).first << "_value ]";
			}
		...
		if (!requiredParams.empty()) {
			...
			write_docu_list(requiredParams, "- <span class=\"paramname\">",
					cscan_.param_list_, runMode_.auto_add_params, "</span>");
			fout_ << "* .\n  ";
		}
		if (!optionalParams.empty()) {
			fout_ << "* <i>Optional Parameters for varargin:</i>\n  ";
			write_docu_list(optionalParams, "- <span class=\"paramname\">",
					cscan_.param_list_, runMode_.auto_add_params, "</span>");
			fout_ << "* .\n  ";
		}
