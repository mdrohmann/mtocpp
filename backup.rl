...
/*    cout << p << "x" << tmp_p << endl;*/
  ...
  garble_comment_line = (any - '\n')* . '\n' @{ cout << "/* "; cout.write(tmp_p, p - tmp_p) << "*/\n"; };

  WSOC = ( [ \t]+ | ('%' @{ tmp_p = p+1; } . garble_comment_line) | ('...'.[ \t]*.'\n') );
  ...
  funcline := |*
    ...
    ((IDENT %{tmp_string.assign(ts,p-ts);}) . '.' . (IDENT >(st_tok) ))
    {
  ...
  *|;
  ...
  funcbody := |*
      ...
      (comment_block) { cout.write(tmp_p, p - tmp_p+1); fcall in_comment_block; };
...
/*        cout << "*"; cout << res << "  "; */
    ...
    # begin required_list
    ( doc_begin . [ \t]*
      . /required fields of /i . (IDENT >(st_tok) %(string_tok) )
      . [ \t]* . ':' . [ \t]* . '\n' )
    {
      //cout << tmp_string << '\n';
    ...
    # begin optional_list
    ( doc_begin . [ \t]*
      . /optional fields of /i . (IDENT >(st_tok) %(string_tok) )
      . [ \t]* . ':' . [ \t]* . '\n' )
    {
  ...
  *|;
  ...
  doxyheader := (
    '%' . [ \t]* .
      ...
      ) $!{p = tmp_p - 2; fgoto doxy_get_brief; }
   );
  ...
  (
    ...
    . [\t]* . 'function'
    . funcdef
  ) $! { cout << "doxyblock for script" << endl; } $eof{ cout << "EOF\n"; end_function(); }
  ;
...
Scanner() : line(1), col(1), ts(0), have(0), top(0), opt(false) {};
...
//        pe -= 50;
        ...
        cout << "eof char" << (int) (*eof) << "newline char" << (int) '\n' << "\n";
  ...
  void end_function()
  {
    ...
    {
      cout << "*\n  ";
      write_docu_list(return_list_, "@param");
      param_list_.clear();
    }
    // return values
